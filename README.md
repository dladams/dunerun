...
./dunerun/build -v &lt;dune-version> -i &lt;install-path> -b &lt;build-path> -q &lt;dune-qualifier>
</pre>
The command-line options may be omitted if the corresponding DUNE build env variables
specified in the [dunerun build convention](./build_convention.md) are all defined.
...
If (any of) the DUNE build env variables are not defined or if the user wishes to define a different env,
then the version (first argument) must be supplied.
Defaqult values are assigned for any omitted arguments: the install location is $HOME/dune/install/%VERSION%,
the build location is /tmp/$USER/dunerun/build/%VERSION% and the qualifier is one appropriate for the version.

The setup file generated by the installation exports the definitions used here for the DUNE
build env variables.
Thus setting up this package (sourcing its setup file) before building subsequent (dune-run-conformant)
